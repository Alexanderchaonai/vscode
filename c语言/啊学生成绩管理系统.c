...
int   Menu(void);
void  ReadScore(STU stu[], int n, int m);//成绩查询
void  AverSumofEveryStudent(STU stu[], int n, int m);//每个学生个人成绩总和和平均分
void  AverSumofEveryCourse(STU stu[], int n, int m);//每个科目的总分和平均分
void  SortbyScore(STU stu[],int n,int m,int (*compare)(float a,float b));//按每个学生的总分排除名次表
int   Ascending(float a, float b);//由低到高
int   Descending(float a, float b);//由高到低
void  SwapFloat(float *x, float *y);//交换成绩
void  SwapLong(long *x, long *y);//交换学号
void  SwapChar(char x[], char y[]);//交换姓名
void  AsSortbyNum(STU stu[], int n, int m);//按学号排序
void  SortbyName(STU stu[], int n, int m);//按姓名排序
void  SearchbyNum(STU stu[], int n, int m);//按学号查询某个学生排名以及考试成绩
void  SearchbyName(STU stu[], int n, int m);//按姓名查询某个学生排名以及考试成绩
void  StatisticAnalysis(STU stu[], int n, int m);
void  PrintScore(STU stu[], int n, int m);//打印成绩表函数
...
// 函数功能：计算每门课程的总分和平均分 
void  AverSumofEveryCourse(STU stu[], int n, int m)//n个学生，m门课
{
	int i, j;
	...
	for(i=0;i<m;i++)//遍历每门课
    {
        for(j=0;j<n;j++)//遍历每个学生，把每个学生该门课程的成绩加到sum[]里面
        {
            sum[i]+=stu[j].score[i];
        }
        aver[i]=sum[i]/n;
...
void SortbyScore(STU stu[],int n,int m,int (*compare)(float a,float b))//传入compare函数的地址，在这个函数里面通过这个地址调用它
{
	int  i, j, k, t;
	for (i=0; i<n-1; i++)
	{
